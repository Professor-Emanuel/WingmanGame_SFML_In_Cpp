...
Enemy::Enemy(dArr<Texture> &textures, Vector2u windowBounds, Vector2f position, 
	Vector2f moveDirection, int type,
...
void Enemy::Update(const float& dt, Vector2f playerPosition)
{
	...
	case FOLLOW:
		...
		this->sprite.move(this->normalizedMoveDir.x * this->maxVelocity * dt * this->dtMultiplier,
	...
	case MOVELEFTSHOOT:
		...
		this->sprite.setRotation(atan2(this->normalizedLookDir.y, this->normalizedLookDir.x) * 
